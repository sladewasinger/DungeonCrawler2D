...
test.describe("camera rotation (Q/X)", () => {
  test("Q rotates the compass + terrain, X rotates it back — no console errors either way", async ({ page }) => {
    ...
    await page.waitForFunction(() => window.__dc2d!.viewOrientation() === 270, undefined, { timeout: 10_000 });
    ...
    await page.waitForFunction(() => window.__dc2d!.viewOrientation() === 0, undefined, { timeout: 10_000 });
    await page.waitForTimeout(200);
