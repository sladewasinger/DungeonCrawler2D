...
describe("wedgeGeometry", () => {
  ...
  it("uses the shipped knife's narrow cone geometry", () => {
    const centerAngle = Math.PI / 3;
    const geo = wedgeGeometry(centerAngle, 50, weaponProfileForId("knife"));
    expect(geo.shape).toBe("cone");
    expect(geo.radiusPx).toBe(81);
    expect(geo.startAngle).toBeCloseTo(centerAngle - Math.acos(0.92));
