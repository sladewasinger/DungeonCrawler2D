...
export function hitPlausiblyFromSwing(swing: PendingSwing, hitX: number, hitY: number): boolean {
  const profile = swing.profile ?? resolveWeaponProfile();
  const dx = hitX - swing.worldX;
  const dy = hitY - swing.worldY;
  const dist = Math.hypot(dx, dy);
  if (dist > profile.range + RANGE_SLACK_TILES) return false;
  if (profile.shape === "ground") return true;
  ...
  return offAxisRad <= Math.acos(profile.arcCos) + ARC_SLACK_RAD;
}
