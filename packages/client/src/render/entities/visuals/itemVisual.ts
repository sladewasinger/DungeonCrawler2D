...
import { groundedVisualPlacement } from "../../../vfx/death/groundPlaneDepth.js";
...
export function createItemVisual(scene: Phaser.Scene): ItemVisual {
  ...
  const label = scene.add.text(0, 0, "", uiTextStyle(10, "#f4d7b2", { scale: 1, weight: "emphasis" }))
...
export function updateItemVisual(visual: ItemVisual, view: ItemEntityView, nowMs: number): void {
  ...
  const placement = groundedVisualPlacement({ rawScreenY: ground.y, groundHeight: view.z, layer: "item" });
  visual.body.setDepth(placement.depth);
  visual.shadow.setDepth(visual.body.depth - 0.2);
  updateShadowPosition({ shadow: visual.shadow, groundScreenX: ground.x, groundScreenY: placement.projectedScreenY });
  if (!view.lootLabel) {
    visual.label.setVisible(false); visual.timer.setVisible(false);
    applyGroundItemMotion({ sprite: visual.body, baseX: ground.x, baseY: placement.projectedScreenY, elapsedMs: nowMs });
    return;
  }
