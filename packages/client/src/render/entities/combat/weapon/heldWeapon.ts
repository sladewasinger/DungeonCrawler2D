...
export const ATTACK_READY_TINT = 0xffffff;
export const MULTIPLY_TINT_MODE = 0;
export const FILL_TINT_MODE = 1;
...
export function updateHeldWeapon(sprite: Phaser.GameObjects.Sprite, frame: string | null, pose: HeldWeaponPose): void {
  if (!frame) {
    sprite.clearTint().setTintMode(MULTIPLY_TINT_MODE).setVisible(false);
    return;
  }
...
function applyHeldWeaponTint(sprite: Phaser.GameObjects.Sprite, pose: HeldWeaponPose): void {
  if (pose.attackReadyFlash) {
    sprite.setTint(ATTACK_READY_TINT).setTintMode(FILL_TINT_MODE);
    return;
  }
  if (pose.blocking) {
    sprite.setTint(BLOCK_GUARD_TINT).setTintMode(MULTIPLY_TINT_MODE);
    return;
  }
  if (pose.isFistFallback) {
    sprite.setTint(FIST_TINT).setTintMode(MULTIPLY_TINT_MODE);
    return;
  }
...
function setWeaponDepth(sprite: Phaser.GameObjects.Sprite, pose: HeldWeaponPose): void {
  sprite.setDepth(depthForCombatOverlay(pose));
}
