...
function weaponProbe() {
  return {
    ...
    setTintMode(value: number) { this.tintMode = value; return this; },
...
describe("updateHeldWeapon blocking", () => {
  ...
  it("flashes the currently held weapon white, then restores its normal tint", () => {
    const probe = weaponProbe();
    updateHeldWeapon(
      probe as unknown as Phaser.GameObjects.Sprite,
      "weapon_sword",
      { ...BLOCKING_POSE, attackReadyFlash: true },
    );
    expect(probe.tint).toBe(ATTACK_READY_TINT);
    expect(probe.tintMode).toBe(FILL_TINT_MODE);

    updateHeldWeapon(
      probe as unknown as Phaser.GameObjects.Sprite,
      "weapon_sword",
      { ...BLOCKING_POSE, attackReadyFlash: false },
    );
    expect(probe.tint).toBe(BLOCK_GUARD_TINT);
    expect(probe.tintMode).toBe(MULTIPLY_TINT_MODE);
  });

  it("clears a ready flash when the weapon is hidden while downed", () => {
    const probe = weaponProbe();
    probe.tint = ATTACK_READY_TINT;
    ...
    expect(probe.tint).toBe(0);
    expect(probe.tintMode).toBe(MULTIPLY_TINT_MODE);
  });

  it("draws elevated weapons above immediate screen-south terrain", () => {
    const probe = weaponProbe();
    updateHeldWeapon(
      probe as unknown as Phaser.GameObjects.Sprite,
      "weapon_sword",
    ...
    expect(probe.depth).toBeGreaterThan(depthForOccluder(1));
