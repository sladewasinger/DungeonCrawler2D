...
export function drawSubtleSlope(
  ...
  if (edges.north) placeFractionalRect(scene, container, wx, wy, [0, 1], [0, LINE_THICKNESS_FRAC], LINE_COLOR, alpha);
  if (edges.south) {
    placeFractionalRect(scene, container, wx, wy, [0, 1], [1 - LINE_THICKNESS_FRAC, 1], LINE_COLOR, alpha);
  }
  if (edges.west) placeFractionalRect(scene, container, wx, wy, [0, LINE_THICKNESS_FRAC], [0, 1], LINE_COLOR, alpha);
  if (edges.east) {
    placeFractionalRect(scene, container, wx, wy, [1 - LINE_THICKNESS_FRAC, 1], [0, 1], LINE_COLOR, alpha);
  }
}
