...
import { TERRAIN_TILESETS, terrainAtlasFrameName, territoryFloorRole, type TerrainTileRole } from "../planning/tileset.js";
import type { TerrainBatches, TerrainQuadVertices, TerrainSurface } from "../planning/terrainPlanner.js";
...
export interface TerrainDrawQuad { readonly worldTile: { readonly x: number; readonly y: number }; readonly viewTile: { readonly x: number; readonly y: number }; readonly vertices: TerrainQuadVertices; readonly kind: string; readonly height?: number; readonly surface?: TerrainSurface; }

function appendDraw(input: AppendDrawsInput, quad: TerrainDrawQuad): void {
  const role = floorRole(input, quad);
  const atlas = input.options.debug ? TERRAIN_TILESETS.debug : TERRAIN_TILESETS[input.options.biomeAt(quad.worldTile)];
  input.target.push({ atlas, frame: terrainAtlasFrameName(atlas, role, 0), role, variant: 0, worldTile: quad.worldTile, phase: input.phase, depth: drawDepth(role, input.phase, quad.viewTile.y), points: projectQuad(quad.vertices, input.options.projection) });
}

function floorRole(input: AppendDrawsInput, quad: TerrainDrawQuad): TerrainTileRole {
  if (input.defaultRole !== "floor") return input.defaultRole;
  if (quad.surface === "bedrock") return "bedrock";
  if (input.options.debug) return input.defaultRole;
  return territoryFloorRole(input.options.territoryAt?.(quad.worldTile));
}
...
function appendFeatureDraw(target: TerrainAtlasDraw[], quad: TerrainBatches["features"][number], options: TerrainAtlasRenderOptions): void {
  ...
  target.push({ atlas, frame: terrainAtlasFrameName(atlas, quad.feature, 0), role: quad.feature, variant: 0, worldTile: quad.worldTile, phase: 1, depth, points: projectQuad(quad.vertices, options.projection) });
}
