...
export class ThreeTouchControls {
  ...
  private build(): void {
    ...
    mountTouchControls({
      ...
      stick: this.stick,
      knob: this.knob,
      aimStick: this.aimStick,
      aimKnob: this.aimKnob,
      beginStick: this.beginStick.bind(this),
      moveStick: this.moveStick.bind(this),
      endStick: this.endStick.bind(this),
      beginLook: this.beginLook.bind(this),
      moveLook: this.moveLook.bind(this),
      endLook: this.endLook.bind(this),
      queueJump: () => this.queueJump(),
      triggerAction: (action) => this.triggerAction(action),
      setInteractHeld: (held) => { this.state.interactHeld = held; },
      setBlock: (held) => { this.state.block = held; },
      setRun: (held) => { this.state.run = held; },
      setJump: (held) => { this.state.jump = held; },
      setJumpButton: (button) => { this.jumpButton = button; },
    });
    this.built = true;
  }
