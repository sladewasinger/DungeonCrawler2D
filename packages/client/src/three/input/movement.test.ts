...
describe("stepFirstPerson", () => {
  it("moves along the camera's forward direction", () => {
    const result = stepFirstPerson({
      state: { x: 0.5, y: 0, z: 0.5, verticalVelocity: 0, grounded: true },
      input: { forward: 1, right: 0, jump: false, yaw: 0 },
      world: world(() => 0), seconds: 0.1,
    });
  ...
  it("does not climb terrain higher than the step limit", () => {
    const result = stepFirstPerson({
      state: { x: 0.5, y: 0, z: 0.5, verticalVelocity: 0, grounded: true },
      input: { forward: 1, right: 0, jump: false, yaw: 0 },
      world: world((_x, z) => (z < 0.5 ? 2 : 0)), seconds: 0.1,
    });
  ...
  it("does not automatically climb a one-tile ledge", () => {
    const result = stepFirstPerson({
      state: { x: 0.5, y: 0, z: 0.5, verticalVelocity: 0, grounded: true },
      input: { forward: 1, right: 0, jump: false, yaw: 0 },
      world: world((_x, z) => (z < 0.5 ? 1 : 0)), seconds: 0.1,
    });
  ...
  it("steps onto a half-tile ledge", () => {
    const result = stepFirstPerson({
      state: { x: 0.5, y: 0, z: 0.5, verticalVelocity: 0, grounded: true },
      input: { forward: 1, right: 0, jump: false, yaw: 0 },
      world: world((_x, z) => (z < 0.5 ? 0.5 : 0)), seconds: 0.1,
    });
