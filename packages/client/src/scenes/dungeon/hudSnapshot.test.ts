...
describe("buildHudSnapshot", () => {
  it("maps health/ping/connection straight through", () => {
    const snap = buildHudSnapshot(source({ hp: 5, maxHp: 30, pingMs: 99 }), null, null);
  ...
  it("fills hotbar counts from inventory stacks", () => {
    ...
    const snap = buildHudSnapshot(source({ hotbar, inventory }), null, null);
  ...
  it("highlights the hotbar slot holding the equipped weapon", () => {
    ...
    const snap = buildHudSnapshot(source({ hotbar, weapon: "sword" }), null, null);
    expect(snap.selectedSlot).toBe(1);
  });

  it("selects -1 when unarmed", () => {
    const snap = buildHudSnapshot(source({ weapon: null }), null, null);
    expect(snap.selectedSlot).toBe(-1);
  });

  it("resolves buff kind/duration from content, defaulting unknown ids to a debuff", () => {
    const snap = buildHudSnapshot(source({ fx: ["on-fire", "regenerating", "made-up"] }), null, null);
  ...
  it("picks party as the active channel once partied", () => {
    const snap = buildHudSnapshot(source({ hasParty: true }), null, null);
    expect(snap.activeChatChannel).toBe("party");
  });

  it("passes through armedThrowableSlot and interactionPrompt unchanged", () => {
    const prompt = { key: "E", label: "interact" };
    const snap = buildHudSnapshot(source(), 3, prompt);
