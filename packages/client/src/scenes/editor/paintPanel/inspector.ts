...
// finite height, cap, stair direction, door) plus the bench read (Epic 7.11) — a painted
...
/** "height z2", "floor (medieval-sewer:1)", "stairs dir=0", "door" — the paint-over
...
function stackText(store: EditorStore, x: number, y: number): string {
  ...
  if (stack.feature) return `${stack.feature} (floor z${stack.height})`;
  if (stack.cap !== null) return stack.height > 0 ? `floor cap (${stack.cap}) at z${stack.height}` : "floor";
  return `void at z${stack.height}`;
}
...
export function inspectorText(store: EditorStore, x: number, y: number, projectedFace?: InspectorFace): string {
  ...
  const tileName = store.world.stackAt(x, y).cap === null ? "Void" : cell.tile === TILE.DoorSafeRoom ? "Door" : "Floor";
