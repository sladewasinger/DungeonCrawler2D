// First-launch showcase, in the explicit-height floor/stairs vocabulary: stepped
// terraces with floor caps, a safe room done RIGHT
// this time (z2 perimeter height, corners closed — the known user gripe with the old
...
/** Raises then floor-caps every cell in the rect — a solid raised platform. */
function stackAndCapRect(
  ...
  for (let y = y0; y <= y1; y++) {
    for (let x = x0; x <= x1; x++) {
      for (let i = 0; i < layers; i++) world.paintHeightAt(x, y);
      world.paintFloorAt(x, y, capId);
    }
  }
}

/** Perimeter height (corners included — closed, not skipped) stacked to `layers`.
 * Interior cells are left untouched (bare ground). */
function heightPerimeter(world: EditableWorld, x0: number, y0: number, x1: number, y1: number, layers: number): void {
  for (let y = y0; y <= y1; y++) {
    for (let x = x0; x <= x1; x++) {
...
function stampStaircase(
  ...
  for (let dx = 0; dx < width; dx++) {
    ...
    for (let i = 0; i < topLayers; i++) world.paintHeightAt(x0 + dx, yBottom - steps);
...
export function seedDemoPattern(world: EditableWorld): void {
  // Stepped terraces: raised floors capped with a real pack floor variant at each tier.
  ...
  heightPerimeter(world, 11, 11, 17, 16, 2);
