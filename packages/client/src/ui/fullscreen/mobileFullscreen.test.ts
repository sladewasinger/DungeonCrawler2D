...
interface FullscreenOptions { standard?: boolean;

webkit?: boolean;

active?: "standard" | "webkit";

enabled?: boolean;

}

function fullscreenRequest(enabled: boolean | undefined): (() => Promise<void>) | undefined {
  return enabled ? () => Promise.resolve() : undefined;

}

function activeFullscreenPart(options: FullscreenOptions, target: object): object {
  if (options.active === "standard") return { fullscreenElement: target };

  if (options.active === "webkit") return { fullscreenElement: null, webkitFullscreenElement: target };

  return { fullscreenElement: null };

}

function fakeDocument(options: FullscreenOptions = {}) {
  const listeners = new Map<string, Listener>();

  const target = {
    requestFullscreen: fullscreenRequest(options.standard),
    webkitRequestFullscreen: fullscreenRequest(options.webkit),
  ...
  const doc = {
    ...
    ...activeFullscreenPart(options, target),
...
describe("mobile fullscreen", () => {
  ...
  it("arms exactly one unmodified game tap after a mobile return", async () => {
    ...
    const retry = installFullscreenResumeRetry({ target: asElement(target), doc: asDocument(doc), deviceScreen: asScreen({}), touchDevice: true });
