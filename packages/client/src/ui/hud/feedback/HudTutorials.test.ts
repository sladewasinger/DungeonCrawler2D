...
import { HudTutorials } from "./HudTutorials.js";
...
interface ConnectionState {
  ...
  party: null;
  pendingInvite: null;
  outgoingPartyInvites: Map<string, string>;
  body: { x: number; y: number };
  entities: Map<string, never>;
}
...
describe("HudTutorials", () => {
  ...
  it("renders a polite borderless hint above the hotbar with gentle motion", () => {
    ...
    const tutorials = new HudTutorials("keyboard");
  ...
  it("dismisses low-health guidance on recovery or bandage depletion", () => {
    ...
    const recovered = new HudTutorials("keyboard");
  ...
  it("replays persisted hydration and selected-action hints", () => {
    installBrowser();
    const firstHud = new HudTutorials("keyboard");
    ...
    const reloadedHud = new HudTutorials("keyboard");
