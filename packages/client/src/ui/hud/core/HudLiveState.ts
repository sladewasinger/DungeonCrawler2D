...
import { activeLootChestNearby, nearestLootChest } from "../../../net/queries/lootChestQuery.js";
import { nearestDownedPartyMember } from "../../../scenes/dungeon/world/contentQueries.js";
import {
  craftSnapshot,
  stashSnapshot,
} from "../../../scenes/dungeon/hud/hudSnapshot.js";
import {
  resolveInteractionPrompt,
  type InteractionPrompt,
} from "../../../scenes/dungeon/world/interactionPrompt.js";
...
function noticeState(connection: Connection, world: World, selectedSlot: number): HudNoticeState {
  return {
...
export interface HudLiveStateSyncRequest {
  readonly connection: Connection;
  readonly world: World;
  readonly selectedSlot: number;
  readonly panels: HudPanels;
  readonly notices: HudNotices;
  readonly closeCraft: () => void;
  readonly closeStash: () => void;
}

export function syncHudLiveState(request: HudLiveStateSyncRequest): void {
  const { connection, world, selectedSlot, panels, notices, closeCraft, closeStash } = request;
  const state = buildHudLiveState(connection, world, selectedSlot);
