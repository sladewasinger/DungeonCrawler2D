...
import { PANEL_BORDER, PANEL_FILL, SELECTION_ACCENT, spacing } from "../../../foundation/panel.js";
...
export class InteractionPromptWidget {
  ...
  constructor(scene: Phaser.Scene, registry: WidgetRegistry, viewport: Viewport) {
    ...
    this.keyText = scene.add
      .text(0, PANEL_HEIGHT / 2, "", uiTextStyle(13, keyColor, { scale: layout.scale, weight: "emphasis" }))
      .setOrigin(0, 0.5);
    this.labelText = scene.add.text(0, PANEL_HEIGHT / 2, "", uiTextStyle(13, undefined, { scale: layout.scale })).setOrigin(0, 0.5);
  ...
  update(prompt: { key: string; label: string } | null): void {
    ...
    this.keyText.setText(
      `[${inputModality.current === "touch" ? "USE" : prompt.key}]`,
    );
