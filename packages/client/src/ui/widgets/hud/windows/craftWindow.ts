...
import type { CraftSnapshot, ToastData } from "../core/fakeData.js";
...
export class CraftWindowWidget {
    ...
    constructor({ scene, registry, viewport, actions }: CraftWindowOptions) { this.scene = scene;
...
const title = scene.add.text(0, -PANEL_HEIGHT / 2 + TITLE_HEIGHT / 2, "CRAFTING", uiTextStyle(12, undefined, { scale: this.scale, weight: "emphasis" })).setOrigin(0.5, 0.5);
...
this.panel.setVisible(false);
    ...
    private rebuildRows(recipes: readonly RecipeRowView[]): void { for (const obj of this.rowObjects)
        obj.destroy();
this.rowObjects = [];
const top = -PANEL_HEIGHT / 2 + TITLE_HEIGHT + spacing(1);
recipes.slice(0, MAX_VISIBLE_ROWS).forEach((view, index) => { this.rowObjects.push(...this.buildRow(view, top + index * ROW_HEIGHT));
});
}
    private buildRow(view: RecipeRowView, y: number): Phaser.GameObjects.GameObject[] { const left = -PANEL_WIDTH / 2 + spacing(1);
...
const icon = createItemIcon({ scene: this.scene, itemId: view.outputId, size: ICON_SIZE, containerScale: this.scale }).setPosition(left + ICON_SIZE / 2 + 4, y + ICON_SIZE / 2 + 4);
const name = this.scene.add.text(left + ICON_SIZE + spacing(1.5), y + 6, `${view.outputName} ×${view.outputQty}`, uiTextStyle(11, undefined, { scale: this.scale })).setOrigin(0, 0);
const ingredients = this.scene.add.text(left + ICON_SIZE + spacing(1.5), y + 22, ingredientsLabel(view), uiTextStyle(9, view.craftable ? MET_COLOR : UNMET_COLOR, { scale: this.scale })).setOrigin(0, 0);
...
return objects;
    ...
    private buildCraftButton(view: RecipeRowView, x: number, y: number): Phaser.GameObjects.GameObject[] { const bg = this.scene.add.rectangle(x, y, CRAFT_BTN_WIDTH, CRAFT_BTN_HEIGHT, PANEL_FILL, 0.9).setOrigin(0, 0).setStrokeStyle(1, PANEL_BORDER);
const text = this.scene.add.text(x + CRAFT_BTN_WIDTH / 2, y + CRAFT_BTN_HEIGHT / 2, "Craft", uiTextStyle(10, view.craftable ? "#e8e8e8" : "#6b6b7e", { scale: this.scale })).setOrigin(0.5, 0.5);
if (view.craftable) {
        bg.setInteractive({ useHandCursor: true });
        bg.on("pointerdown", () => this.actions.craft(view.recipeId));
    } return [bg, text];
}
    update(craft: CraftSnapshot, lastToast: ToastData | null, nowMs: number): void { if (!this.open)
        return;
if (!craft.nearby) {
        this.close();
        return;
...
function ingredientsLabel(view: RecipeRowView): string { return view.ingredients.map((i) => `${i.need}× ${i.name} (${i.have}/${i.need})`).join("   ");
}
