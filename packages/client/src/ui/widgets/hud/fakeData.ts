...
export interface HudFakeSnapshot {
  ...
  respawnRemainingSec: number; respawnHoldProgress: number; downedRemainingSec: number; reviveProgress: number; reviverName: string | null;
...
const FAKE_CHAT_MODEL: ChatPanelModel = {
  ...
  lines: [
    ...
    // Unbroken run: proves character-level (advanced) wrap defeats WWWW grief.
    { channel: "local", author: "Grief", text: "W".repeat(120) },
...
export function fakeHudSnapshot(downed: boolean): HudFakeSnapshot {
  return {
    ...
    downed,
    dead: false,
    respawnRemainingSec: 24, respawnHoldProgress: 0.55, downedRemainingSec: 30, reviveProgress: 0, reviverName: null,
