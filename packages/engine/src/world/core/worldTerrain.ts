...
export class WorldTerrain {
  ...
  replaceTileOverrides(overrides: readonly { x: number; y: number; tile: TileType }[]): void {
    if (!this.access.voidTerrain && overrides.some(({ tile }) => tile === TILE.Void)) {
  ...
  isWalkable(wx: number, wy: number): boolean {
    ...
    if (SOLID_TILES.has(this.surfaceTileAt(wx, wy))) return false;
...
const featureHeightOverride = (tile: TileType, heightAt: () => number): number =>
  featureFromTile(tile) === TILE.Floor ? 0 : heightAt();
