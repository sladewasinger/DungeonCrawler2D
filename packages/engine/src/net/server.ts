...
export const bodySnapshotSchema = z.object({ x: z.number(), y: z.number(), z: z.number(), zVel: z.number(), grounded: z.boolean(), coyoteTime: z.number().nonnegative(), jumpBuffer: z.number().nonnegative(), jumpHeld: z.boolean(), kx: z.number(), ky: z.number() });

export const activeStatusSnapshotSchema = z.object({ id: z.string(), remainingSeconds: z.number().nonnegative().nullable(), durationSeconds: z.number().positive().nullable(), stacks: z.number().int().positive().optional() });
export type ActiveStatusSnapshot = z.infer<typeof activeStatusSnapshotSchema>;

export const selfSnapshotSchema = bodySnapshotSchema.extend({ hp: z.number(), maxHp: z.number(), movementSpeed: z.number().nonnegative().optional(), stamina: z.number().nonnegative().optional(), maxStamina: z.number().positive().optional(), blocking: z.boolean().optional(), staminaRecoveryDelaySeconds: z.number().nonnegative().optional(), staminaExhausted: z.boolean().optional(), healthRegenerationDelaySeconds: z.number().nonnegative().optional(), fx: z.array(z.string()), statusEffects: z.array(activeStatusSnapshotSchema).optional(), downed: z.boolean().optional(), downedUntilTick: z.number().int().nullable().optional(), reviveProgress: z.number().min(0).max(1).optional(), reviverName: z.string().optional(), respawnAtTick: z.number().int().nullable().optional(), xp: z.number().int().nonnegative().optional(), level: z.number().int().positive().optional(), xpForNext: z.number().int().nonnegative().optional(), floor: z.number().int().positive().optional(), deepestFloor: z.number().int().positive().optional() });

export const entitySnapshotSchema = z.object({ id: z.string(), kind: z.enum(["player", "enemy", "pet", "item", "projectile", "torch"]), defId: z.string().optional(), name: z.string().optional(), skin: z.enum(PLAYER_SKINS).optional(), x: z.number(), y: z.number(), z: z.number(), hp: z.number().optional(), maxHp: z.number().optional(), fx: z.array(z.string()).optional(), qty: z.number().optional(), downed: z.boolean().optional(), reviveProgress: z.number().min(0).max(1).optional(), disconnected: z.boolean().optional(), anim: enemyAnimationStateSchema.optional(), petOwnerName: z.string().optional(), aimX: z.number().min(-1).max(1).optional(), aimY: z.number().min(-1).max(1).optional(), faceX: z.number().min(-1).max(1).optional(), faceY: z.number().min(-1).max(1).optional(), weapon: z.string().nullable().optional(), blocking: z.boolean().optional(), air: z.literal(true).optional(), vx: z.number().optional(), vy: z.number().optional(), vz: z.number().optional(), state: z.enum(["flying", "placed"]).optional(), expiresAtTick: z.number().int().optional(), lootOwnerName: z.string().optional(), lootKillerId: z.string().optional(), lootKillerName: z.string().optional(), lootUnlockAtTick: z.number().int().optional() });
