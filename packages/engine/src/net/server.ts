...
export const activeStatusSnapshotSchema = z.object({ id: z.string(), remainingSeconds: z.number().nonnegative().nullable(), durationSeconds: z.number().positive().nullable() });
export type ActiveStatusSnapshot = z.infer<typeof activeStatusSnapshotSchema>;

export const selfSnapshotSchema = bodySnapshotSchema.extend({ hp: z.number(), maxHp: z.number(), stamina: z.number().nonnegative().optional(), maxStamina: z.number().positive().optional(), blocking: z.boolean().optional(), staminaRecoveryDelaySeconds: z.number().nonnegative().optional(), staminaExhausted: z.boolean().optional(), healthRegenerationDelaySeconds: z.number().nonnegative().optional(), fx: z.array(z.string()), statusEffects: z.array(activeStatusSnapshotSchema).optional(), downed: z.boolean().optional(), downedUntilTick: z.number().int().nullable().optional(), reviveProgress: z.number().min(0).max(1).optional(), reviverName: z.string().optional(), respawnAtTick: z.number().int().nullable().optional(), xp: z.number().int().nonnegative().optional(), level: z.number().int().positive().optional(), xpForNext: z.number().int().nonnegative().optional(), floor: z.number().int().positive().optional(), deepestFloor: z.number().int().positive().optional() });
