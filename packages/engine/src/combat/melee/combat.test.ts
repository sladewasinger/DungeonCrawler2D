...
describe("melee targeting aid", () => {
  it("prefers a hostile over a closer party member in the same arc", () => {
    ...
    const target = pickMeleeTarget({ attacker, direction: { x: 1, y: 0 }, candidates: [friend, enemy], isPartyMember: (target) => target.partyId === "party1" });
  ...
  it("hits the party member when no hostile is in the arc — friendly fire is real", () => {
    ...
    const target = pickMeleeTarget({ attacker, direction: { x: 1, y: 0 }, candidates: [friend], isPartyMember: (target) => target.partyId === "party1" });
  ...
  it("respects range, arc, and vertical reach", () => {
    ...
    expect(pickMeleeTarget({ attacker, direction: { x: 1, y: 0 }, candidates: [behind, far, above], isPartyMember: () => false })).toBeNull();
  });
  ...
  it("never targets items or the dead", () => {
    ...
    expect(pickMeleeTarget({ attacker, direction: { x: 1, y: 0 }, candidates: [corpse, loot], isPartyMember: () => false })).toBeNull();
  });
});
...
describe("enemy AI", () => {
  it("wanders without a target, chases when a player is in aggro range", () => {
    ...
    const wander = enemyThink({ brain, enemy: slime, def: slimeDef, players: [farPlayer], inSanctuary: () => false, dt: 0.05, rng: () => rng.next() });
    expect(wander.strike).toBeUndefined();

    const nearPlayer = combatant("player", 5, 0);
    const chase = enemyThink({ brain, enemy: slime, def: slimeDef, players: [nearPlayer], inSanctuary: () => false, dt: 0.05, rng: () => rng.next() });
  ...
  it("ignores players inside sanctuary — safe rooms are safe", () => {
    ...
    const decision = enemyThink({ brain, enemy: slime, def: slimeDef, players: [shopper], inSanctuary: () => true, dt: 0.05, rng: () => rng.next() });
  ...
  it("spitters shoot from range instead of closing in", () => {
    ...
    const decision = enemyThink({ brain, enemy: spitter, def: spitterDef, players: [player], inSanctuary: () => false, dt: 0.05, rng: () => rng.next() });
  ...
  it("ignores downed players (they're out of the fight)", () => {
    ...
    enemyThink({ brain, enemy: slime, def: slimeDef, players: [downed], inSanctuary: () => false, dt: 0.05, rng: () => rng.next() });
    expect(brain.targetId).toBeNull();
  });
