...
import { assertLiveGraceCase } from "./seedSweepSupport.js";
...
const SEED_COUNT = 20;
const SEED_SWEEP_TIMEOUT_MS = 180_000;
  ...
  it(
    ...
    () => {
      for (let seed = 1; seed <= SEED_COUNT; seed++) {
        const world = new World(hashString(`spawn-seed-sweep-${seed}`), 1, LEVEL.Dungeon);
        ...
        const join = addPlayer(sim, { name: "Seed sweep", clientId: `client-${seed}` });
