import { encodeMessage, type ServerSnapshot } from "@dc2d/engine";
...
export function broadcastTick(floors: FloorRegistry, sandbox: GameSim, sockets: SocketMap): void {
  const { snapshots, moved } = floors.stepAll();
  for (const { playerId, sim } of moved) {
    const entry = sockets.get(playerId);
    if (entry) entry.sim = sim;
  }
  sendSnapshots(snapshots, sockets);
  sendSnapshots(sandbox.step(), sockets);
}

function sendSnapshots(snapshots: Map<string, ServerSnapshot>, sockets: SocketMap): void {
