...
function clusterCenter(
  ...
  const dir = DIAG[hash2D(mixSeeds(seeds.layout, 0x9e5b), cx, cy) % 4]!;
  const clamp = (v: number) => Math.max(REACH, Math.min(CHUNK_SIZE - 1 - REACH, Math.round(v)));
  return { lx: clamp(jlx + dir[0] * 8), ly: clamp(jly + dir[1] * 8) };
}
 ...
 * the flattened features, before the test zone / custom map / sealing).
 */
  ...
  for (let ly = centerLy - REACH; ly <= centerLy + REACH; ly++) {
    for (let lx = centerLx - REACH; lx <= centerLx + REACH; lx++) {
      ...
      if (d <= PAD) {
        ...
        height[i] = padH + (height[i]! - padH) * smooth;
      }
    }
  }
}
