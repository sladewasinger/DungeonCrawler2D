import type { EnemyDef } from "../effects/types";
import type { Entity } from "../entities/entity";
import type { MoveInput } from "../entities/movement";
...
export function enemyThink(
  ...
  // Acquire/validate target: nearest living player outside sanctuary.
  ...
  if (!target) {
    // Wander: pick a direction for a while, sometimes stand still.
    brain.wanderLeft -= dt;
    if (brain.wanderLeft <= 0) {
      brain.wanderLeft = 1 + rng() * 2;
      brain.wanderDir = {
        moveX: AXIS[Math.floor(rng() * 3)]!,
        moveY: AXIS[Math.floor(rng() * 3)]!,
        jump: false,
      };
    }
