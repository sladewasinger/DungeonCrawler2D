...
- [ ] **Network movement remains smooth:** Open two clients in both 2D and Three mode. Walk, stop, jump, cross chunk boundaries, briefly background one tab, reconnect it, and observe the other. **Expected:** local movement stays immediate, remote actors move smoothly without cyclic drift/teleports, reconnect preserves identity/state, and no renderer-specific desync appears.
