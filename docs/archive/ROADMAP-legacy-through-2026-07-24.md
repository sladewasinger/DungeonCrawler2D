...
8. [ ] **2D presentation/content:** cohesive four-direction crawler art, readable kill
   ...
   **Live tuning and session controls:** move player-facing balance and presentation
   constants out of scattered code into a validated, versioned configuration surface
   with sensible defaults, persistence, and a restricted admin page that applies
   authoritative gameplay changes live and broadcasts the revised values to connected
   ...
   **Mobile HUD-edit and controls ergonomics:** HUD Edit Mode must be practically
   ...
   panel. Shift the default movement joystick approximately 50 px right to reserve
   reliable left-side space for sprint input, then validate the resulting landscape
   layout on real touch-sized viewports.
   **Mobile action and combat polish:** make Throw, Use, and Jump match the current
   Attack-button size, then enlarge Attack by 25% while preserving non-overlapping
...
- [x] **PvP rules per [GAME_DESIGN.md](GAME_DESIGN.md):** everyone can damage everyone — friendly fire is always on, with half direct melee damage between party members; `sanctuary` suppresses everything
...
- [x] Death: **full loot drop** where you fell (anyone may loot — including your killer), respawn at random distant location, keep stash; downed-then-revive for party members with a 60 s bleed-out
...
- [x] Party system: invite/accept via proximity (mutual consent, [F] fistbump flow), leave/disband, party chat channel, and labeled direction/distance tracking from member position pings outside AOI (friendly fire stays on — no flag)
...
- [x] **Pickup:** interacting with a still-burning placed torch restores exactly one inventory torch, no partial-durability tracking (#8); burned-out torches despawn rather than persisting as a second "spent" entity state. A 40-torch per-floor cap (#9), reserving both flying and placed torches before inventory consumption, bounds worst-case rebake cost.
- [x] Fading-ember visual tell in a placed torch's last ~15s (#7, deferred from the wave-2 build)
...
- [x] **Rate limiting:** one recoverable 5 messages/rolling-3s cap across all channels and `/who`; floor transfers reset mismatched tick-clock history so throttling cannot become permanent (ASSUMPTIONS #46)
...
- [x] **Party downed/revive UX:** the 60s bleed-out revive runs server-side (`deaths.ts`). `partySnapshotSchema` includes `hp`/`downed` so off-AOI party frames can show it, with an on-screen revive prompt when standing near a downed party member and a deliberate 1.5s hold-[K] give-up path into ordinary respawn (#32)
...
- [x] **Dedicated title/mode-select screen:** Phaser and Three.js share an explicit Dungeon/Sandbox choice with consequence descriptions; choosing a mode resumes that mode's stored session rather than silently guessing between tokens (#36).
- [ ] **Session-menu parity:** no production in-game route back to mode selection exists yet. Ship the non-pausing menu scheduled in active queue item 6 with a disconnecting `Quit to opening screen` action; do not infer parity from the old reference implementation (#36).
...
- [x] Protocol-mismatch UX: protocol_mismatch is terminal for the stale connection and opens a renderer-neutral "Update required" refresh prompt instead of reconnecting forever.
...
- [x] **Bandage healing contract:** a bandage never presents as damage. Applying one immediately restores **4 HP** and shows green floating **`+4`** feedback (never red `-4`), then applies or refreshes a visible healing/regeneration buff for exactly 5 seconds that restores **2 HP per second** (14 HP total, clamped to max health). The server snapshots authoritative remaining/total status time; both HUDs share its presentation model. All balance values originate in validated version-1 `liveTuning.json`.
...
- [x] **Health bars only after damage** (DCC-book style): bars stay hidden on first observation, reveal on a real damage edge, remain visible while injured, and hide when full or dead.
...
- [x] **Torch pop-in residue:** newly activated torch halos ease into the additive pool over 250ms, so budget/stream activation reads as kindling rather than a pop.
...
- [x] Coords readout uses room-chunk-relative x/y inside reserved room instances while simulation and overworld coordinates remain unchanged.
...
- [x] Brightness round 4: ambient 0.62 -> 0.72 with a COOL blue-gray cast at the floor and warm gold at torches (hue contrast restores torch presence — "torches seem to do nearly nothing"), halo alpha 0.45 -> 0.62; melee wedge fill 0.25 -> 0.45 so the pie visibly starts at the player's feet (point-blank enemies are inside it — the hit math always included them)
- [ ] **Walk-behind + occlusion outline (user request):** investigate the blocked tile at his screenshot coords (x-72,y-47 wanting y-46) — determine whether a legal floor tile sits behind the wall's visual face; implement the standard top-down treatment: floor rows behind tall walls stay walkable, the player renders as a silhouette outline while occluded by wall geometry. **Silhouette shipped** (new `render/entities/occlusion.ts`; live-probed at the reported coords — the wall band at y-47/-46 is a real height-1.0 solid tile, not a rendering artifact; see `docs/ASSUMPTIONS.md` #111, proof `docs/client-proofs/wave7b-occlusion.png`). **"stay walkable" generation fix still open** — no legal floor tile exists at (x-72,y-47) itself, and fixing that is a worldgen/terrain-legibility change outside this lane's owned files
...
- [ ] **ELEVATION PROJECTION (user rulings 2026-07-20, THE unifying render decision — supersedes the flat-projection entity commit fcb4530 and the wall-gap item's local-anchoring framing):** the user's two rulings pin the projection model uniquely: (1) grounded entities stand ON their drawn tiles (platform test: the unlifted orb was right, the floating sprite wrong), AND (2) losing height moves you DOWN-SCREEN ("if I jump off the left side of a cliff [z1 to z0] I should land further down visually... kind of behind that wall to my south, slightly occluded" — same x,y before/after, z 1->0, seed 228182761 ~x20,y-39). Only whole-scene elevation projection satisfies both: TERRAIN SURFACES DRAW SHIFTED NORTH BY THEIR HEIGHT (cap of a z1 platform renders at worldY-1) and entities draw at worldY - z; grounded, those coincide ON the shifted tile; drops move south on screen; tall terrain naturally occludes what is behind it. The debug renderer currently draws terrain FLAT at world rows (placeDebugTile) — the flat/shifted war between terrain and entities is the single root of the sprite-float, shadow, wedge, wall-gap, and cliff-drop reports. Rework scope: terrain draw paths place at height-shifted rows (through the rotation seam: the shift is screen-up at every orientation), exposed south rows render face fill, occluder/cliff-mask interplay, editor render panel same shift, CLICK-PICKING needs a height-disambiguation rule (a screen point can be tall-far or low-near), camera/AOI margins, minimap unchanged (world-space). Entities revert to worldY - z (absolute z-lift) once terrain shifts — the fcb4530 flat convention was interim. Design contract first (single opus designer, read-only), build after user sees the plan. STAIR DIRECTIONALITY RULING (user 2026-07-20, seed 228182761 ~x45,y-12 z0.4, screenshots at 0 and 180): a N->S high-to-low stair currently renders IDENTICALLY at opposite orientations — "they look like they're going UP not down... they shouldn't be drawn, or they should be drawn differently." The projection contract must make the two orientations render distinguishably: descending-toward-camera (high end screen-top) shows the full tread run stretched across its shifted rows; descending-away (high end screen-bottom) compresses/hides behind the high side's face — plus the approved split-straddle tint asymmetry (high half rim-tinted, low half pit-tinted, nosing line) so direction reads even in a still frame
- [ ] **OUTLINE SCOPE CORRECTION (user ruling 2026-07-20, build e217244 playtest, SUPERSEDES the item below's "every side" wording):** white cliff-edge outlines belong to RAISED WALKABLE FLOORS ONLY — the perimeter (all exposed sides) of platform/cap surfaces, "sides of raised floors is what I intended". WALL bodies NEVER get white side edges (the vertical white lines on wall flanks looked wrong; walls' visual language stays the black bitmask border). Implementation: keep cliffMask.ts but guard the face-cell consumer with tileAt !== Wall (or equivalent walkable-surface check). NOTE: the stairs lane reverted drawTile.ts/topEdges.ts at 20:13 wiping the first outline wiring — re-land post-wave with this scope, by the orchestrator inline
- [ ] **WORLDGEN — corridor minimum width (user question 2026-07-21, ORCHESTRATOR RECOMMENDS 2-WIDE EVERYWHERE, awaiting his nod):** 1-wide passageways read as gaps between wall masses in the flat style (his screenshot: an E-W tunnel indistinguishable from a seam), and camera rotation erases any N-S/E-W distinction — an axis-specific rule is meaningless post-rotation. Recommendation: generator carves ALL corridors >=2 wide (carve-width parameter + invariant test: every corridor cell has an adjacent parallel walkable cell); optionally keep RARE deliberate 1-wide "crawlspaces" later as DCC-flavored set pieces, but never as the default artery. Deliberate worldgen re-baseline (goldens move again). Alternative he floated (visual passageway affordance on 1-wides) judged weaker: space reads better than signage in this style
- [ ] **PERF+ROBUSTNESS — RT-pipeline pressure (judge panel #3 lead + user stutter report, 2026-07-21):** (a) movement stutters persist — chunk bake cost grew with the projection's per-row strip model (592 RenderTextures in one probe view); consolidate strips (merge adjacent same-depth rows, or atlas strips per chunk) and time-slice bakes ahead-of-camera; (b) judges' WebKit-GL context hit "Framebuffer status: Incomplete Attachment" at Phaser boot -> terminal boot error, 100% reproducible THERE (prod boots fine on real Chrome + headless chromium smoke): treat as a mobile-Safari-shaped risk (WebKit = the stated mobile target) — verify RT creation on a WebKit/ANGLE-diverse pass, and add graceful degradation on framebuffer-creation failure (reduce RT usage / retry smaller / canvas fallback) instead of a dead "please reload" loop
- [ ] **PERFORMANCE RELEASE — network, boot, and bake budget (user direction 2026-07-23):** ordered work: (1) spatially bucket server entities so AOI snapshots inspect only intersecting buckets, while retaining deterministic ordering; (2) then introduce revisioned, backwards-compatible deltas for inventory/hotbar and unchanged entities, with a full-baseline recovery path; (3) independently lazy-load Phaser and Three.js renderer entrypoints — the current 2D main payload is ~1.57 MB and must become a Phaser-only lazy chunk, with manifests proving a chosen route never downloads the other renderer; (4) continue pooling transient Phaser visuals and spread a single chunk bake across frames only if measured hardware exceeds the frame budget. Existing strip-atlas/page pooling and urgency-tiered cross-chunk baking are complete. Acceptance: server benchmark proves AOI work scales with nearby buckets, idle snapshots reduce bytes/allocations without stale actors or reconnect regressions, route bundles are isolated, and fixed-seed visual output remains unchanged while walk-route p99/max frames improve or hold.
- [ ] **WORLDGEN — minimum pit size (user question 2026-07-20, undecided):** playtest found a cramped 1-wide pit (seed 228182761 ~x-50,y11) where the old multi-tile stair blocked walking and forced a jump-climb. With compact 1-tile stairs a 1x1 pit is just a hole filled by its own stair, and 1xN trenches read as noise. RECOMMENDATION (not yet user-approved): generator culls pits with interior smaller than 2x2 (fill flat); keep larger pits. User said "I don't know if that is the right call" — present after the stairs wave lands so he judges with the new stairs in hand (user ruling 2026-07-20, branch build d006f65, seed 228182761, example tile 15,40 at z1.0):** raised platforms are hard to read as raised — wall stacks get black bitmask outlines on all exposed sides, but CLIFF edges (raised floor dropping to lower ground without a wall) only get the white/light edge line on SOUTH-facing edges; west/east (and any exposed) sides of single raised tiles show nothing, and nearby black-outlined walls make the difference actively confusing. Ruling: outline the ENTIRE exposed cliff perimeter in white — every side whose neighbor height is lower (his example: west AND east of 15,40). Implement with the same lower-neighbor mask the rim-occlusion item computes (one shared cliff mask feeds outlines + occluders); world-space like autotile borders, so correct at all 4 rotation orientations for free
- [ ] **WAVE R2 — south-wall approach gap + occlusion at elevated z (user playtest 2026-07-20, LIVE REPRO: build 58e3eb1, seed 4097222748, x -24, y 91, z 1.0, pushing south):** standing at z1, his southernmost reachable position leaves a visible GAP to the south wall — the shadow never touches the wall visual, and he never renders "behind" it. Two entangled causes to separate with a live probe AT THOSE COORDS before fixing: (a) the shadow-on-z0-plane bug (item below) draws his position marker 1 tile south of his true lifted feet at z1, so the readable position is wrong to begin with; (b) possibly a REAL collision-vs-visual mismatch — an invisibly-rendered blocking stack between his platform and the visible wall (the invisible-wall bug class), or wall visuals north-lifted relative to their collision cell. Expected correct behavior per his ruling: he walks flush to the wall cell's edge and his sprite's southern portion is OCCLUDED by the taller wall's face — which is the SAME generalized rule as floor-rim occlusion: any terrain higher than the entity between it and the camera occludes, at every z, not just in pits. Probe on the branch (collision height field is byte-identical to main), fix through the view seam. SECOND, SHARPER REPRO (2026-07-20, branch build d006f65, seed 228182761, ~x5,y16,z1 left platform vs ~x9,y16,z1 right platform): the user isolated the variable — at a CLIFF edge (right platform) he reaches the true south edge, position reads correct, and stepping past falls perfectly; at a WALL boundary (left platform) holding S stops him ~1 TILE NORTH of the visible purple wall-face edge, where he believes walkable z1 floor technically continues ("~1 tile south... which would slightly occlude me if I could move further south like I should be able to"). So cliff approach = correct, wall approach = short by ~1 tile. Probe must decide between: (a) the height field HAS walkable z1 floor south of his stop and collision wrongly blocks it -> collision/height bug; (b) the stop is physically correct but the wall's face/cap is DRAWN ~1 tile too far south relative to the lifted platform frame (face anchored by the wall's own height lift, mismatched against the viewer floor's z-lift) -> renderer anchoring bug. ACCEPTANCE (user's words): walking south into a wall on a platform ends FLUSH with the visible face edge, character slightly occluded by it; cliff edges keep behaving exactly as they do now. THIRD REPRO — THE ANCHOR TRIANGLE (2026-07-20, build 211e3d5+tree, seed 228182761, x13, y-46, z1.0): standing at z1, the personal light HALO draws on the z0 plane (never lifted — same family as the fixed shadow/wedge), the SPRITE+SHADOW draw z-lifted ~1 tile north "floating on the side of the wall", and the user reads the HALO's spot as where he actually is ("they should be where the white glow is — my character and shadow floating on the side of the wall makes 0 sense"). The probe MUST resolve which anchor agrees with the DRAWN walkable surface at those coords (dump heights, compute where the platform cap renders, compare sprite/shadow/halo screen anchors) — then make ALL THREE (sprite, shadow, halo) land on the drawn surface underfoot. If the drawn cap disagrees with the user's read, the CAP anchoring is what changes (his read is acceptance), which likely ALSO explains repro 1/2's one-tile wall-face offset — suspect a systematic 1-tile north bias between terrain lift rendering and entity lift rendering at z>0 a z0→z-1 stairway is ~3 tiles long (thirds-quantized steps forced by STEP_UP 0.35) and "it's just a pain to get back on it" from inside the pit. User ruling: stairs should be 1 TILE long (or at most a rim-straddling 2×half-tile treatment), NOT a multi-tile runway eating the pit floor. Implementation: stair tiles become TRUE RAMP SURFACES — continuous height interpolation across the single tile from high edge to low edge, with grounded movement glued to the surface (STEP_UP no longer the binding constraint on designated stairs). This is a real engine+worldgen change that DELIBERATELY breaks the byte-identical worldgen invariant (re-baseline it); generator pit exits become one stair tile flush against the rim; editor/stacks stair paint semantics + fixture migration follow. Design spec in docs/R2-STAIRS-SPEC.md (design panel output). USER APPROVED both open questions 2026-07-20 ("That all sounds good for now"): chasm (z-2) edges go SHEER (no descending death-slide stair; bridges cross, falling in stays lethal) and the stair renders the SPLIT-STRADDLE treatment (upper half rim-tinted, lower half pit-tinted, bright nosing line at the seam)
- [ ] **WAVE R2 — shadow/ground unification (same playtest):** the shadow blob renders on the z0 plane, so at z≠0 the sprite and shadow diverge (in a pit the shadow floats near the player's head; he has to line up the SHADOW, not the sprite, to climb stairs). Rule: shadow lifts by the LOCAL GROUND height beneath the entity — grounded ⇒ shadow exactly at the feet at every z; the ONLY legitimate divergence is airborne (jump: shadow stays on the ground while the sprite rises). Same view-seam routing as above

### Performance release status (2026-07-23)
...
## PANEL ROUND 3b (2026-07-21, prod a8cff15-era) — 3x REJECT, convergent + buildable

Scores (polish/fun/playable): Grinder 6/5/4 (kill-chain blocker CLEARED — witnessed -5 float, kill line, blood decal, XP fill); Tourist 6/5/4 (REVOKED round-2 accept); BookFan 6/6/5 (descent still ungated — by WAYFINDING, not content). Universally praised: instant chainable rotation + letter compass, DCC announcer voice ("the best writing in the build"), onboarding text.

Ranked blockers (the R3 wave):
- [ ] **1. SPAWN SAFETY (server sim; 2 judges):** hostiles can occupy/threaten the entry tile at control-handoff; death respawns into the same ambush — pre-input deaths on 3/7 joins. Fix in sim spawn/respawn: enforce a no-hostile clearance radius at the entry/respawn tile at handoff (relocate loiterers or shift the slot) + consider a ~2s spawn grace. Fuzz test across seeds/joins.
- [ ] **2. STAIRS WAYFINDING (BookFan's round-2+3 blocker):** floor-1 stairway unfindable in 90 min of instrumented hunting. Add a diegetic signal: gold stairway tick on the compass dial (rotation-aware for free) + optional proximity cue. This gates verifying the ENTIRE descent chain (floors, scaling, Warden).
- [ ] **3. ELEVATION LEGIBILITY SHOWCASE (Tourist's revoke):** raised terrain needs rim + drop-shadow treatment (= the user's pre-ordered fake-AO polish item), pits a visibly recessed dark floor; plus guarantee one unambiguous platform and one pit within the first minutes of spawn as the showcase.
- [ ] **4. WALL-BUMP FEEDBACK + SILHOUETTES:** blocked movement gives zero cue (all 3 judges hit silent walls); add a nudge/flash/thud deny cue; differentiate wall vs fence vs corridor silhouettes (composes with the pending 2-wide corridor call).
- [ ] **5. WHIFF FEEDBACK + FLOOR-1 COST:** distinct miss-vs-connect cue on swings; first-kill cost 25/30 HP is brutal for floor-1 trash — tune toward a fair first fight.
- [ ] **Small:** "audience rates it a 6 out of 10" NEVER varies — derive the rating from run stats (kills/floor/duration) so the audience reads as reactive; COMPASS caption clips to "COMPAS" at every viewport width (label offset math).

## PANEL ROUND 4 (2026-07-21, deploy 5f21e76) — 3x REJECT, R3 fixes landed PARTIALLY
Grinder 6/5/4 (spawn: death->chip, 2/7 joins pre-input HP loss — enemies DRIFT BACK during the 2s grace; clearance is handoff-instant only). Tourist 5/4/4 (AO imperceptible at 0.5 in cold read; showcase not found near either spawn; wall-bump works but subtle; dead in 80s incl. chat-freeze ranged hits). BookFan 6/4/4 (STILL no Floor 2, 3 rounds running — the gold tick bears THROUGH walls; maze pockets defeat bearings; death rating showed "5/10" on all 3 numbered deaths + 2 deaths had no number; NEW: respawn comes back UNARMED — starter sword not re-granted; grace skin reads as a slime on your head).
R4 (unblocked, building now): spawn exclusion maintained THROUGH grace + clearance-after-population ordering; respawn re-grants starter kit; rating always numbered + spread widened; grace visual = shield ring not teal blob; ground-truth probe (prod-seed spawn heights, showcase presence, AO 0.5-vs-1.0 comparison shots for the user's slider call).
GATED ON USER: (a) 2-wide corridors + pocket-density reduction — now blocks THREE judges (wayfinding + lethality funnels), recommendation stands; (b) AO strength — his workbench slider, needs his eyes on the comparison shots.
