...
- [ ] **PRE-MERGE CLEANUP (before pivot branch merges to main):** prune the dead pack-art RENDER paths left unused by the debug-tileset re-skin (packArt.ts pickDoorTile + any packSprite draw paths no longer imported) — the pack CATALOG stays (user direction: packs parked for floor 2+ theming), but unreferenced render code does not ride into main; also sweep for any other unimported modules with knip-style dead-export check
