...
- **CHASM_DEATH_Z (−1.5):** the −1 pit's low anchor (−1) is above it, so gliding to the pit floor does not trigger the server death ruling — a legitimately escapable pit. **Invariant:** a stair intended to be walkable in both directions must have `low anchor > CHASM_DEATH_Z`; the generator must never author such a stair descending into a true void (−2). Void cells are now infinite-height collision boundaries, so a body cannot walk or jump onto them; the server death path remains for bodies knocked or teleported into one.
...
4. Void room: no ramp; the bridge is the sole walkable crossing and black, flat void cells have no projected faces.
...
- `world/stacks/compile.test.ts` explicit-`stair.height` override and feature/floor cases; editor `EditableWorld.test.ts` current stack-map round trips.
...
1. **Chasm (−2) exit — sheer edge.** Chasm rooms get **no descending terrain-stair**. Their bridge is the only walkable crossing; the rim is an infinite-height void boundary. A body knocked or teleported into the void still follows the server death path, while ordinary pits/daises retain compact walkable stairs.
