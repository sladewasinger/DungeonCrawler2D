...
- **Assets are baked binaries**: `tools/generate-art.mjs` (`npm run art`) procedurally draws the tile atlas + player sheet as committed PNGs, and emits `atlas.json` (frame indices) that the client imports — one source of truth. Hand-drawn art later replaces the generator's output behind the same atlas contract
- **Chunks render as Phaser tilemap layers** (base terrain + overlay for cliff faces and ledge rims) sharing the single atlas texture on the GPU — at 64px/tile, per-chunk canvases would be 2048² textures; tilemaps keep memory flat. Height shading is per-tile tint; variant selection is `hash2D(wx, wy)`, so every client draws the identical world
